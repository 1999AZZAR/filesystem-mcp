...
/**
 * MCP 2026 Stateless HTTP Transport Adapter for FileSystem MCP (Membrane).
 */
  ...
  async start(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.httpServer!.listen(this.options.port, this.options.host, () => {
        console.error(
          `FileSystem MCP Server running on HTTP (${this.options.host}:${this.options.port}, stateless MCP 2026)`
        );
        resolve();
      });
